...
export default function BullishFlag() {
  const t = useTranslations(
    "public.learn.technicalAnalysis.chapter1.lessons.bullish.topics.bullish_flag.content",
  );

  const getDefinitionData = (): DefinitionData => {
    const rawDefinition = t.raw("definition");
    const images = [IMAGES.CHART_PATTERN.BULLISH.BULLISH_FLAG.DEFINITION];
  ...
  const getExamplesData = (): ExampleData[] => {
    ...
    const charts = [[[IMAGES.CHART_PATTERN.BULLISH.BULLISH_FLAG.EXAMPLE]]];
